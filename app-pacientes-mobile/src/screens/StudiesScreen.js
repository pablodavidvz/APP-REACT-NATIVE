...
const StudiesScreen = () => {
  ...
  const loadStudies = async () => {
    try {
      const data = await patientService.getPatientStudies(patient.dni);
  ...
  return (
    <ScrollView className="flex-1" style={{ backgroundColor: theme.colors.background }} contentContainerStyle={{ padding: 16 }}>
      <View className="items-center mb-6">
        ...
        <Text style={{ color: theme.colors.text }} className="text-2xl font-bold mt-2">Estudios Médicos</Text>
      </View>
      ...
      {studies.length === 0 ? (
        <View className="items-center py-12">
          ...
          <Text style={{ color: theme.colors.textSecondary }} className="text-center mt-4">
            No hay estudios médicos disponibles
          </Text>
        </View>
      ) : (
        studies.map((study, index) => (
          <View key={index} className="rounded-xl p-4 mb-4" style={{ backgroundColor: theme.colors.card }}>
            <Text style={{ color: theme.colors.text }} className="font-bold">{study.name}</Text>
            <Text style={{ color: theme.colors.textSecondary }} className="text-sm">{study.date}</Text>
          </View>
        ))
      )}
    </ScrollView>
